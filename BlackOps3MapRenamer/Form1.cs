...
namespace BlackOps3MapRenamer {
	public partial class Form1 : Form {
        ...
        private async Task<List<string>> RenameZoneOccurrences() {
			...
			foreach (string line in lines) {
				...
				if (!originalLine.Equals(updatedLine)) {
					AppendColoredTextSafe("[ZONE] ", Color.FromArgb(255, 128, 0));
					AppendColoredTextSafe(originalLine, Color.Maroon);
					AppendColoredTextSafe(" -> ", Color.Black);
					AppendColoredTextSafe(updatedLine + "\n", Color.DarkGreen);
				}
			}
		...
		private async Task<List<string>> RenameSoundZoneConfigOccurrence()
		{
			...
			foreach (string line in lines)
			{
				...
				if (updatedLine.Contains("\"Name\" : \"" + _mapName))
				{
					...
					AppendColoredTextSafe("[SOUND ZONE CONFIG] " + line + " -> " + updatedLine + "\n", Color.DarkOliveGreen);
                }
				newLines.Add(updatedLine);
            }
						...
						if (File.Exists(assetListPath)) {
							...
							foreach (string csvLine in csvLines) {
								...
								if (!csvLine.Equals(updatedLine)) {
									changed = true;
									AppendColoredTextSafe("[ZONE_SOURCE, " + lang + ", ASSETLIST] " + csvLine + " -> " + updatedLine + "\n", Color.DarkCyan);
								}
							}
							if (changed) {
								try {
					...
					if (Directory.Exists(assetInfoDir)) {
						...
						if (File.Exists(assetInfoPath)) {
							...
							foreach (string csvLine in csvLines) {
								...
								if (!csvLine.Equals(updatedLine)) {
									changed = true;
									AppendColoredTextSafe("[ZONE_SOURCE, " + lang + ", ASSETINFO] " + csvLine + " -> " + updatedLine + "\n", Color.DarkMagenta);
								}
							}
							if (changed) {
								try {
		...
		private async Task<List<string>> RenameAllLangOccurrences() {
			...
			if (File.Exists(assetListCSVFilePath)) {
				...
				foreach (string line in lines) {
					...
					if (!line.Equals(updatedLine)) {
						changed = true;
						AppendColoredTextSafe("[ZONE_SOURCE, ALL, ASSETLIST] " + line + " -> " + updatedLine + "\n", Color.DarkCyan);
					}
				}
			...
			if (File.Exists(assetInfoCSVFilePath)) {
				...
				foreach (string csvLine in lines) {
					...
					if (!csvLine.Equals(updatedLine)) {
						changed = true;
						AppendColoredTextSafe("[ZONE_SOURCE, ALL, ASSETINFO] " + csvLine + " -> " + updatedLine + "\n", Color.DarkMagenta);
					}
				}
				if (changed) {
					try {
		...
		private async Task<List<string>> RenameLangOccurrences(string lang)
		{
			...
			if (File.Exists(assetListCSVFilePath))
			{
				...
				foreach (string line in lines)
				{
					...
					if (!line.Equals(updatedLine)) {
						changed = true;
						AppendColoredTextSafe("[ZONE_SOURCE, " + lang.ToUpper() + ", ASSETLIST] " + line + " -> " + updatedLine + "\n", Color.DarkCyan);
					}
				}
			...
			if (File.Exists(assetInfoCSVFilePath))
			{
				...
				foreach (string csvLine in lines)
				{
					...
					if (!csvLine.Equals(updatedLine)) {
						changed = true;
						AppendColoredTextSafe("[ZONE_SOURCE, " + lang.ToUpper() + ", ASSETINFO] " + csvLine + " -> " + updatedLine + "\n", Color.DarkMagenta);
					}
				}
				if (changed) {
					try {
		...
		private void Form1_Load(object sender, EventArgs e) {
			if (Properties.Settings.Default.usermap_folder != null && Properties.Settings.Default.usermap_folder.Length != 0) {
				if (Directory.Exists(Properties.Settings.Default.usermap_folder)) {
					folderBrowserDialog1.SelectedPath = Properties.Settings.Default.usermap_folder;
				}
			} else
			{
				if (IsAdministrator())
				{
					folderBrowserDialog1.SelectedPath = GetSpecificSteamLibraryPath();
				}
			}
